...
impl EnigoManager {
    ...
    /// 模拟粘贴快捷键（Windows/Linux 为 Ctrl+V，macOS 为 Cmd+V）。
    /// 用于「点选剪贴板历史 -> 直接粘贴到当前应用」。
    pub fn paste(&mut self) -> Result<(), String> {
        ...
        if let Err(e) = enigo.key(modifier, Direction::Press) {
            return Err(format!("[EnigoManager::paste] press modifier error: {}", e));
        }

        let click_result = enigo.key(Key::Unicode('v'), Direction::Click);

        // 无论 v 是否成功，都要释放修饰键，避免修饰键卡住
        let release_result = enigo.key(modifier, Direction::Release);

        if let Err(e) = click_result {
            return Err(format!("[EnigoManager::paste] click v error: {}", e));
        }
        if let Err(e) = release_result {
            return Err(format!(
                "[EnigoManager::paste] release modifier error: {}",
                e
            ));
        }
