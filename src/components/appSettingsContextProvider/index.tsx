...
const AppSettingsContextProviderCore: React.FC<{
		...
		(
			...
			if (group === AppSettingsGroup.Common) {
				...
				settings = {
					...
					themePreset:
						typeof newSettings?.themePreset === "string"
	...
	const reloadAppSettings = useCallback(async () => {
		...
		// 始终用磁盘最新值发布，保证跨窗口（如剪贴板侧栏）能拿到主窗口刚改的设置。
	...
	const themePreset = useMemo(
		() => getThemePreset(appSettings[AppSettingsGroup.Common].themePreset),
		[appSettings],
	);
	const appContextValue = useMemo(() => {
		return {
			appWindowRef,
			// ZMENG 主题预设主导明暗，兼容旧的 currentTheme 消费方（dark/light）
			currentTheme: themePreset.isDark
	...
	const antdTheme = useMemo((): ThemeConfig => {
		const algorithms = [
			themePreset.isDark ? theme.darkAlgorithm : theme.defaultAlgorithm,
		];
		...
		return {
			...
			token: {
				// 主题预设的 token（含主色/表面色）优先，圆角仍可用户自定义
				...themePreset.token,
	...
	useEffect(() => {
		document.body.className = themePreset.isDark ? "app-dark" : "app-light";
		// 注入 CSS 变量，供自定义样式组件（如剪贴板侧栏、首页仪表盘）跟随主题
		const root = document.documentElement;
		const v = themePreset.cssVars;
		root.style.setProperty("--zmeng-bg", v.bg);
		root.style.setProperty("--zmeng-surface", v.surface);
		root.style.setProperty("--zmeng-surface-hover", v.surfaceHover);
		root.style.setProperty("--zmeng-elevated", v.elevated);
		root.style.setProperty("--zmeng-text", v.text);
		root.style.setProperty("--zmeng-text-secondary", v.textSecondary);
		root.style.setProperty("--zmeng-border", v.border);
		root.style.setProperty("--zmeng-primary", v.primary);
		root.style.setProperty("--zmeng-primary-soft", v.primarySoft);
	}, [themePreset]);
