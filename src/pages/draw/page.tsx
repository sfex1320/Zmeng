...
const DrawPageCore: React.FC<{
	...
	/** 临时调整常驻置顶的剪贴板侧栏的置顶状态，让截图覆盖层可以盖在其之上 */
	const setSidebarTopmost = useCallback(async (topmost: boolean) => {
		try {
			const { Window } = await import("@tauri-apps/api/window");
			const sidebar = await Window.getByLabel("clipboard-sidebar");
			await sidebar?.setAlwaysOnTop(topmost);
		} catch {
			// 侧栏未创建/不可用时忽略
		}
	}, []);
	...
	const showWindow = useCallback(
		async ({ min_x, min_y, max_x, max_y }: ElementRect) => {
			...
			await Promise.all([
				...
				// 截图优先级高于剪贴板侧栏：降级侧栏置顶，使覆盖层可盖住它（像素已在截图位图中）
				setSidebarTopmost(false),
			]);
		...
		[getScreenshotType, setSidebarTopmost],
	);

	const hideWindow = useCallback(async () => {
		await appWindowRef.current.hide();
		// 恢复剪贴板侧栏的常驻置顶
		setSidebarTopmost(true);
		// await Promise.all([
